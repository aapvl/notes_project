...
@pytest.fixture()
def auth_headers_test_user(user_admin):
    user_data = {"username": "testuser", "password": "1234", "role": "admin"}
    headers = {
...
def test_note_get_by_id(client, note, auth_headers_test_user):
    response = client.get('/notes/1', headers=auth_headers_test_user)
